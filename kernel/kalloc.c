...
struct 
{
  ...
  int mem_count[(PHYSTOP - KERNBASE)/PGSIZE];
}mem_ref_struct;

int get_mem_count(uint64 pa){
  int count; 
  acquire(&mem_ref_struct.lock);
  count = mem_ref_struct.mem_count[((uint64)pa - KERNBASE)/ PGSIZE];
  release(&mem_ref_struct.lock);
  return count;
}
void mem_count_up(uint64 pa){
  acquire(&mem_ref_struct.lock);
  ++ mem_ref_struct.mem_count[((uint64)pa - KERNBASE)/ PGSIZE];
  release(&mem_ref_struct.lock);
}
...
int mem_count_down(uint64 pa){
  int flag = 0;
  acquire(&mem_ref_struct.lock);
  if((-- mem_ref_struct.mem_count[((uint64)pa - KERNBASE)/ PGSIZE]) == 0){
    flag = 1;
  }
  release(&mem_ref_struct.lock);
  return flag;
...
void mem_count_set_one(uint64 pa){
  acquire(&mem_ref_struct.lock);
  mem_ref_struct.mem_count[((uint64)pa - KERNBASE)/ PGSIZE] = 1;
  release(&mem_ref_struct.lock);
}
